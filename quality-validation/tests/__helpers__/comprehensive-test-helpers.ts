...
export const ROBUST_SELECTORS = {
  auth: {
    emailInput: 'input[type="email"], #email, [data-testid="email-input"]',
    passwordInput: 'input[type="password"], #password, [data-testid="password-input"]',
...
export async function loginWithCredentials(
  ...
  try {
    ...
    // Wait for form elements with better error handling using robust selectors
    await page.waitForSelector(ROBUST_SELECTORS.auth.emailInput, { timeout: TEST_TIMEOUTS.MEDIUM });
    await page.waitForSelector(ROBUST_SELECTORS.auth.passwordInput, { timeout: TEST_TIMEOUTS.MEDIUM });
    
    // Fill form fields
    await page.fill(ROBUST_SELECTORS.auth.emailInput, email);
    await page.fill(ROBUST_SELECTORS.auth.passwordInput, password);
    
    // Submit form using robust selector
    await page.click(ROBUST_SELECTORS.auth.loginButton);
...
} export function measurePageLoadTime() { return Promise.resolve(100); }
